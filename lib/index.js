...
function generateXML (data, isCustomDate){
    ...
    channel.push({ lastBuildDate:   isCustomDate ? data.lastBuildDate : new Date().toUTCString() });
    ...
    ifTruePush(data.pubDate, channel, { 'pubDate': isCustomDate ? data.pubDate : new Date(data.pubDate).toUTCString() });
    ...
    data.items.forEach(function(item) {
        ...
        ifTruePush(item.link || item.guid, item_values, { guid:         [ { _attr: { isPermaLink: !item.guid && !!item.url } }, item.guid || item.url ]  });
        ...
        ifTruePush(item.date, item_values, { pubDate:      isCustomDate ? item.date : new Date(item.date).toUTCString() });
...
function RSS (options, items) {
    ...
    this.xml = function(options) {
        const isCustomDate = options ? options.isCustomDate : null;
        const indent = options ? options.indent : {};
        return '<?xml version="1.0" encoding="UTF-8"?>' +
            xml(generateXML(this, isCustomDate), indent);
    };
}
